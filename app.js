(() => {
  ...
  async function loadAllFields() {
    ...
    try {
      // ใช้ getCell เดิม 6 ครั้งแบบพร้อมกัน เพื่อให้ทำงานได้กับ Deployment เดิม
      // ไม่ต้องพึ่งคำสั่ง getAllCells ใน Apps Script เวอร์ชันใหม่
      const cells = ['A2', 'B2', 'C2', 'D2', 'E2', 'F2'];
      const results = await Promise.all(
        cells.map(cell => bridgeCall('getCell', { cell }))
      );

      const data = {};
      cells.forEach((cell, index) => {
        data[cell] = results[index]?.value ?? '';
      });
